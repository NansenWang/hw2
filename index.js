...
function save(){
 ...
 arr=[];
  ...
  var isDone=(mainUl.find('span').parents('li').hasClass("is-done")) ? true : false;
    arr.push({
      "text":$(this).text(),
      "isDone":isDone
    });
	//console.log(mainUl.find('span'));
  ...
  if(localStorage.todoItems){
	...
	for(i=0;i<arr.length;i++){
		...
		if (arr[i]["isDone"]===true){
 			$(tmpl).prependTo(mainUl).addClass('is-done').appendTo(mainUl).find('span').text(arr[i]["text"]);
		}else{
